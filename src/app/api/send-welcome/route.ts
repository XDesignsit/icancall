...
export async function POST(request: Request) {
  try {
    ...
    const result = await sendEmail({
      ...
      text: `Hi ${name || 'there'},\n\nWelcome to iCanCall! Your account is ready to use.\n\nBest,\nThe iCanCall Team`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
          ...
          <div style="margin-top: 24px;">
            <a href="https://yourdomain.com/dashboard" style="display: inline-block; padding: 12px 24px; background-color: #2563eb; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 500;">Go to Dashboard</a>
          </div>
