"use client";

import Link from "next/link";

export default function NotFound() {
  return (
    <div className="min-h-screen flex flex-col">
      {/* Styles for animation and responsive utilities not covered by tailwind config */}
      <style dangerouslySetInnerHTML={{ __html: `
        .logo-main .cls-1 { fill: #1c2530; }
        .logo-main .cls-2 { fill: #4083ae; }
        .logo-main .cls-3 { fill: #fff; }

        @keyframes pulse {
          0% { box-shadow: 0 0 0 0 oklch(0.58 0.115 232 / 0.40); }
          70% { box-shadow: 0 0 0 16px oklch(0.58 0.115 232 / 0); }
          100% { box-shadow: 0 0 0 0 oklch(0.58 0.115 232 / 0); }
        }

        .animate-pulse-custom::after {
          content: "";
          position: absolute;
          inset: 0;
          border-radius: 28px;
          box-shadow: 0 0 0 0 oklch(0.58 0.115 232 / 0.45);
          animation: pulse 2.4s ease-out infinite;
        }

        .nf-code span {
          color: var(--blue-deep, oklch(0.44 0.105 240));
        }
      `}} />

      <header className="header border-b border-[var(--line)]">
        <div className="wrap header-inner flex items-center justify-between h-[72px]">
          <Link href="/" className="brand flex items-center gap-[11px]" aria-label="iCanCall home">
            <svg className="logo-main" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 553.0305" style={{ height: "32px", width: "auto", display: "block" }}>
              <g>
                <path className="cls-1" d="M707.4397,239.6996l-.4398-.6591c-31.0327-46.2476-71.4038-97.0542-117.1563-115.2897-5.2177-2.4717-6.9757-4.9985-2.5277-9.777,4.9441-5.1081,11.2601-11.0948,14.6112-17.796,20.8722-36.6356-8.074-84.4763-50.1482-82.2791-44.1058.769-68.9324,53.1134-43.0062,88.0463,4.1744,6.7561,14.6648,13.4569,16.3129,17.4664.8783,2.3621-.2749,3.9548-2.6913,5.8225-22.9037,11.9189-41.9093,33.4498-63.4398,49.1585-24.9366,18.51-48.3902.5495-73.1069-12.9625-9.777-4.6686-13.7865-8.4035-5.9311-18.8946,20.4873-28.2321-1.3195-70.5248-36.9651-68.438-25.7064.5495-45.2603,25.0466-40.866,50.0929.7147,4.449,2.0879,8.788,4.1744,12.7975,3.7896,8.0743,12.0285,14.226,11.8649,18.8946-.3299,6.0421-9.5584,9.1179-16.8077,15.2146-26.2548,20.4323-47.7867,57.5624-82.9938,31.088-4.7779-3.0758-9.9969-6.5362-14.8847-9.5571-4.6679-3.2404-8.6238-4.8335-9.2272-9.7767-.1649-4.6689,6.9757-11.3697,9.0623-19.7186,8.019-24.7167-16.1479-50.477-41.4694-43.5013-19.1142,4.1195-30.9227,25.7603-24.6617,44.2704,1.868,8.074,10.8752,16.4228,12.5233,20.4873,1.5931,3.6253-2.4714,6.3716-5.5476,8.3489-58.7156,40.4255-120.2325,184.3318-124.0771,280.7269-.6048,22.6295,5.2177,72.887,37.185,64.3185,28.6163-14.0611,45.0941-46.5225,70.4142-67.0098,19.7189-18.3454,45.369-26.3644,67.6693-7.1403,19.9925,14.7201,39.5465,46.2476,68.0528,35.8665,22.8501-8.6781,39.2729-36.4706,61.5719-45.6435,45.4803-17.1369,71.9536,61.7918,117.9274,42.4581,41.688-19.3341,72.832-82.1695,131.5476-53.6079,42.6227,18.7846,78.928,65.8563,121.0022,90.0235,22.1354,12.3584,49.1585,6.6462,64.8117-13.8961,52.9495-82.1145-12.7969-210.1471-52.7832-279.1342ZM687.1173,373.4994l-.3299.879c-27.408,71.2389-106.4474-33.8896-183.4524,14.2257-28.4527,15.7091-55.3659,48.3352-87.3332,28.6166-24.002-15.1047-45.5339-42.7326-76.4016-41.1395-44.6556-.4395-64.868,60.1441-101.7781,51.7952-24.6068-6.8658-46.7421-36.416-76.7315-30.7038-25.2665,1.8676-44.6556,25.8703-68.2727,30.8684-28.0128,1.9226-21.8605-44.5449-18.2358-62.3959,8.074-40.3155,54.4312-15.7057,104.0846-13.2393,25.1566,8.953,45.9188,46.6322,76.0731,31.3079,22.6288-11.7543,44.8192-46.4675,69.9757-58.4414,33.3941-16.972,63.055,12.6879,89.7483,28.8362,22.6852,13.4569,44.1607,4.6689,62.1767-12.6879,44.1607-44.5999,78.9294-77.7201,137.3701-25.2658,42.4027,43.2817,89.4198,108.8085,73.1069,178.3447Z" />
                <path className="cls-3" d="M576.8254,252.827c2.9112,2.0322,5.6575,4.339,8.1839,6.8658-2.4714-2.5267-5.2177-4.8885-8.1839-6.8658ZM574.5189,251.289c-2.2515-1.4281-4.6143-2.6913-7.0857-3.7349,2.4164,1.0986,4.7779,2.3068,7.0857,3.7349ZM544.858,242.9951c-6.096-.1096-11.9735.879-17.4111,2.8013,5.4376-1.8673,11.3151-2.8559,17.4111-2.7463h.8247c5.7675-.055,11.4237.879,16.6977,2.5817-5.3277-1.7577-10.9302-2.6913-16.6977-2.6367h-.8247Z" />
              </g>
              <path className="cls-2" d="M614.0104,195.1547c-58.4407-52.4543-93.2093-19.3341-137.3701,25.2658-18.0159,17.3568-39.4915,26.1448-62.1767,12.6879-26.6933-16.1483-56.3542-45.8081-89.7483-28.8362-25.1566,11.9738-47.3469,46.6871-69.9757,58.4414-30.1543,15.3242-50.9165-22.3549-76.0731-31.3079-49.6534-17.4664-96.0106,93.9237-104.0846,134.2393-3.6246,17.851-9.777,64.3185,18.2358,62.3959,23.6171-4.9981,43.0062-29.0008,68.2727-30.8684,29.9894-5.7122,52.1248,23.838,76.7315,30.7038,36.9101,8.3489,57.1225-52.2347,101.7781-51.7952,30.8677-1.5931,52.3997,26.0349,76.4016,41.1395,31.9673,19.7186,58.8806-12.9075,87.3332-28.6166,77.0051-48.1153,156.0444,57.0133,183.4524-14.2257l.3299-.879c16.3129-69.5362-30.7041-135.0629-73.1069-178.3447ZM161.9688,375.0375c-45.369-4.3394-43.2811-69.9757,2.1978-71.6238h.8783c48.6637,2.2522,45.8638,73.546-3.0762,71.6238ZM378.5432,327.0872c-15.051,43.9408-80.3025,36.9651-85.6302-9.2279-3.6246-25.3758,17.9059-49.653,43.446-49.3785h.7697c29.3296-.4392,51.575,31.0883,41.4144,58.6063ZM601.2122,303.5237c-4.7779,58.1668-84.4756,71.0193-107.5443,17.5764-16.0393-35.592,12.0835-78.6541,51.1901-78.05h.8247c31.8574-.2746,58.5507,28.6716,55.5295,60.4736Z" />
            </svg>
          </Link>
          <div className="header-cta">
            <Link className="btn btn-ghost" href="/">Back to home</Link>
          </div>
        </div>
      </header>

      <main className="nf flex-1 grid place-items-center relative overflow-hidden py-16 px-4 bg-[var(--bg)]">
        {/* Radial gradients background decoration */}
        <div className="absolute inset-[-25%] -bottom-1/2 h-[720px] pointer-events-none z-0 bg-[radial-gradient(58%_58%_at_76%_8%,oklch(0.72_0.095_196_/_0.16),transparent_70%),radial-gradient(54%_54%_at_20%_0%,oklch(0.58_0.115_232_/_0.15),transparent_70%)]" />

        <div className="relative z-10 flex flex-col items-center text-center max-w-[560px]">
          {/* Missed-call badge */}
          <div className="nf-ring animate-pulse-custom w-24 h-24 rounded-[28px] bg-gradient-to-br from-[var(--blue)] to-[var(--teal-deep)] grid place-items-center relative shadow-lg mb-8">
            <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="w-11 h-11">
              <path d="M6.5 3.5h3l1.5 4-2 1.5a12 12 0 0 0 5 5l1.5-2 4 1.5v3a2 2 0 0 1-2.2 2A16 16 0 0 1 4.5 5.7 2 2 0 0 1 6.5 3.5Z" />
            </svg>
            <span className="slash absolute -right-[7px] -bottom-[7px] w-9 h-9 rounded-full bg-[oklch(0.55_0.18_25)] border-3 border-[var(--bg)] grid place-items-center text-white">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" className="w-[17px] h-[17px]">
                <path d="M5 5l14 14" />
              </svg>
            </span>
          </div>

          <div className="nf-code text-[clamp(4.4rem,13vw,8rem)] font-bold leading-[0.92] tracking-tighter text-[var(--ink)] mb-2">
            4<span>0</span>4
          </div>
          <h1 className="text-[clamp(1.7rem,4vw,2.5rem)] font-bold text-[var(--ink)] mb-4">This page didn&apos;t pick up</h1>
          <p className="lead text-[clamp(1.1rem,1.6vw,1.32rem)] text-[var(--ink-soft)] leading-normal max-w-[42ch] mb-9">
            The link you followed rang out — it may have been moved, renamed, or never existed. Let&apos;s route you somewhere that answers.
          </p>

          <div className="nf-actions flex gap-3.5 flex-wrap justify-center mb-12">
            <Link className="btn btn-primary btn-lg rounded-full" href="/">Back to home</Link>
            <Link className="btn btn-ghost btn-lg rounded-full" href="/dashboard">Go to dashboard</Link>
          </div>

          <nav className="nf-route w-full max-w-[420px] bg-[var(--surface)] border border-[var(--line)] rounded-[var(--r-lg)] shadow-md p-[18px_18px_20px] text-left" aria-label="Suggested pages">
            <div className="cap text-[0.74rem] font-semibold tracking-wider uppercase text-[var(--ink-faint)] mb-3 pl-0.5">
              Try these instead
            </div>
            <ul className="list-none flex flex-col gap-2 p-0 m-0">
              <li>
                <Link className="nf-link flex items-center gap-3.25 p-[11px_13px] rounded-[var(--r-md)] border border-[var(--line)] bg-[var(--surface)] hover:border-[var(--blue)] hover:bg-[var(--tint)] hover:shadow-[0_0_0_3px_oklch(0.58_0.115_232_/_0.10)] hover:translate-x-0.5 transition-all duration-200 group" href="/">
                  <span className="avatar w-[38px] h-[38px] rounded-[11px] grid place-items-center text-[var(--blue-deep)] bg-[var(--tint)]">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[19px] h-[19px]">
                      <path d="M3 11.5 12 4l9 7.5" />
                      <path d="M5 10v9h14v-9" />
                    </svg>
                  </span>
                  <span className="who flex-1 min-w-0">
                    <b className="text-[0.97rem] font-semibold block leading-tight text-[var(--ink)]">Home</b>
                    <span className="text-[0.8rem] text-[var(--ink-faint)]">What iCanCall does</span>
                  </span>
                  <span className="go text-[var(--ink-faint)] group-hover:text-[var(--blue-deep)] group-hover:translate-x-[3px] transition-all duration-200">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[18px] h-[18px]">
                      <path d="M5 12h14" />
                      <path d="m13 6 6 6-6 6" />
                    </svg>
                  </span>
                </Link>
              </li>
              <li>
                <Link className="nf-link flex items-center gap-3.25 p-[11px_13px] rounded-[var(--r-md)] border border-[var(--line)] bg-[var(--surface)] hover:border-[var(--blue)] hover:bg-[var(--tint)] hover:shadow-[0_0_0_3px_oklch(0.58_0.115_232_/_0.10)] hover:translate-x-0.5 transition-all duration-200 group" href="/dashboard">
                  <span className="avatar w-[38px] h-[38px] rounded-[11px] grid place-items-center text-[var(--blue-deep)] bg-[var(--tint)]">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[19px] h-[19px]">
                      <rect x="3" y="3" width="7" height="7" rx="1.5" />
                      <rect x="14" y="3" width="7" height="7" rx="1.5" />
                      <rect x="3" y="14" width="7" height="7" rx="1.5" />
                      <rect x="14" y="14" width="7" height="7" rx="1.5" />
                    </svg>
                  </span>
                  <span className="who flex-1 min-w-0">
                    <b className="text-[0.97rem] font-semibold block leading-tight text-[var(--ink)]">Dashboard</b>
                    <span className="text-[0.8rem] text-[var(--ink-faint)]">Manage your call routing</span>
                  </span>
                  <span className="go text-[var(--ink-faint)] group-hover:text-[var(--blue-deep)] group-hover:translate-x-[3px] transition-all duration-200">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[18px] h-[18px]">
                      <path d="M5 12h14" />
                      <path d="m13 6 6 6-6 6" />
                    </svg>
                  </span>
                </Link>
              </li>
              <li>
                <Link className="nf-link flex items-center gap-3.25 p-[11px_13px] rounded-[var(--r-md)] border border-[var(--line)] bg-[var(--surface)] hover:border-[var(--blue)] hover:bg-[var(--tint)] hover:shadow-[0_0_0_3px_oklch(0.58_0.115_232_/_0.10)] hover:translate-x-0.5 transition-all duration-200 group" href="/onboarding">
                  <span className="avatar w-[38px] h-[38px] rounded-[11px] grid place-items-center text-[var(--blue-deep)] bg-[var(--tint)]">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[19px] h-[19px]">
                      <path d="M12 4v16" />
                      <path d="M4 12h16" />
                    </svg>
                  </span>
                  <span className="who flex-1 min-w-0">
                    <b className="text-[0.97rem] font-semibold block leading-tight text-[var(--ink)]">Get started</b>
                    <span className="text-[0.8rem] text-[var(--ink-faint)]">Set up your first call circle</span>
                  </span>
                  <span className="go text-[var(--ink-faint)] group-hover:text-[var(--blue-deep)] group-hover:translate-x-[3px] transition-all duration-200">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[18px] h-[18px]">
                      <path d="M5 12h14" />
                      <path d="m13 6 6 6-6 6" />
                    </svg>
                  </span>
                </Link>
              </li>
            </ul>
          </nav>
        </div>
      </main>
    </div>
  );
}
